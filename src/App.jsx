...
export default function App() {
  ...
  useEffect(() => {
    ...
    const forceShowTimer = setTimeout(markLoaded, 1500);
    ...
    const readWithTimeout = async (key) => {
    ...
    (async () => {
      try {
        const [t, f, l] = await Promise.all([
          readWithTimeout('mf_templates'),
          readWithTimeout('mf_forms'),
          readWithTimeout('mf_logs'),
        ]);

        const parsedTemplates = safeParse(t);
        const parsedForms = safeParse(f);
        const parsedLogs = safeParse(l);

        if (Array.isArray(parsedTemplates)) setTemplates(parsedTemplates);
        if (parsedForms && typeof parsedForms === 'object' && !Array.isArray(parsedForms)) setForms(parsedForms);
